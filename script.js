...
function addListeners() {
    ...
    $(".card-view-expense").on("click", function () {
        ...
        for (const expense of Object.keys(card.expenses)) {
            ...
            expense_item.append(expense_title, expense_info, expense_progress, expense_del);
...
function calculateAmount() {
    ...
    for (let i = 0; i < cards.length; i++) {
        ...
        if (width == 0)
            bar.css("width", "0vw");
        if (width >= 50)
            bar.css("background-color", "darkgoldenrod");
        if (width >= 85)
...
$("#add-expense-btn").on("click", () => {
    ...
    if (card.expenses[expense_name])
        ...
        card.expenses[expense_name] = expense_amount;
    localStorage.setItem("budgets", JSON.stringify([card, ...cards]));
    calculateAmount();
    location.reload();
});
$("#view-budget").on("click", () => {
    ...
    for (let i = 0; i < cards.length; i++) {
        ...
        if (width == 0)
            budget_filler.css("width", "0vw");
        if (width >= 50)
            budget_filler.css("background-color", "darkgoldenrod");
        if (width >= 85)
            budget_filler.css("background-color", "red");
